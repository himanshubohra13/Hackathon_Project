...
from models.classifiers import (predict_malaria, predict_liverD, predict_heartD, predict_alzheimer, predict_diabetes, 
predict_cancerB, predict_glaucoma, predict_covid, predict_brain, localizeTumor, predict_disease, predict_drug)
...
def drugRecommend(request):

    if request.method == 'POST':
        condition = request.POST.get('symptoms0')
        if request.POST.get('drug0'):
            drugname = request.POST['drug0']
            print(condition , drugname)
            rdrugs, res = predict_drug(condition , drugname)
            context = {
                'rdrugs': rdrugs,
                'advice': res
            }
            ...
            rdrugs, res = predict_drug(condition)
            res = 'Please provide any medication that you are following to get any drug recommendations'
            context = {
                'rdrugs': rdrugs,
                'advice': res
            }
        ...
        return render(request, 'medicine/recorder.html')
